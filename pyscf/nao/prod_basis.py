...
libnao.init_vrtx_cc_batch.argtypes = (POINTER(c_double),
        POINTER(c_int64))
...
class prod_basis:
    ...
    def __init__(self, nao=None, **kw):
        ...
        self.tol_biloc = (kw['tol_biloc'] if 'tol_biloc'
                          in kw else 1e-6)
        self.tol_elim = (kw['tol_elim'] if 'tol_elim' in kw else 1e-16)
        self.ac_rcut_ratio = (kw['ac_rcut_ratio'] if 'ac_rcut_ratio'
                              in kw else 1.0)
        self.ac_npc_max = (kw['ac_npc_max'] if 'ac_npc_max'
                           in kw else 8)
        self.pb_algorithm = (kw['pb_algorithm'
                             ].lower() if 'pb_algorithm' in kw else 'pp'
                             )
        self.jcutoff = (kw['jcutoff'] if 'jcutoff' in kw else 14)
        self.metric_type = (kw['metric_type'] if 'metric_type'
                            in kw else 2)
        self.optimize_centers = (kw['optimize_centers'
                                 ] if 'optimize_centers' in kw else 0)
        ...
        if self.pb_algorithm == 'pp':
            ...
            pb_ae(self, nao, self.tol_loc, self.tol_biloc,
                  self.ac_rcut_ratio)
        else:
    ...
    def init_prod_basis_pp(self, sv, **kvargs):
        """
        Talman's procedure should be working well with Pseudo-Potential
        starting point.
        """
        ...
        libnao.init_vrtx_cc_apair(data.ctypes.data_as(POINTER(c_double)),
                                  c_int64(len(data)))
        self.sv_pbloc_data = True

        # going to be some information including indices of atoms,
        # list of contributing centres, conversion coefficients
        self.bp2info = []
    ...
    def init_prod_basis_pp_batch(self, nao, **kw):
        ...
        for (a1, [sp1, ra1]) in enumerate(zip(sv.atom2sp,
                sv.atom2coord)):
            rc1 = aos.sp2rcut[sp1]
        ...
        # going to be indices of atoms, list of contributing centres,
        # conversion coefficients
        self.bp2info = []
        # Conditional fill of the self.bp2info if there are bilocal pairs (natoms>1)
        if npairs > 0:
            ...
            if nao.verbosity > 0:
                ...
                t2 = timer()
                print(__name__,
                      '\t====> Time after vrtx_cc_batch:\t {:.2f} sec'.format(t2
                      - t1))
                t1 = timer()
            nout = 0
            ...
            f = 0
            for (srncc, ndp, npac, [a1, a2]) in zip(p2srncc, p2ndp,
                    p2npac, p2atoms):
                if ndp < 1:
                    continue
    ...
    def init_inp_param_prod_log_dp(self, sv, tol_loc=1e-5, tol_biloc=1e-6,
            ac_rcut_ratio=1.0, ac_npc_max=8, jcutoff=14, metric_type=2,
            optimize_centers=0, ngl=96, **kw):
        """
        ...
        self.sv = sv
        (self.tol_loc, self.tol_biloc, self.ac_rcut_ratio,
         self.ac_npc_max) = (tol_loc, tol_biloc, ac_rcut_ratio,
                             ac_npc_max)
        (self.jcutoff, self.metric_type, self.optimize_centers,
         self.ngl) = (jcutoff, metric_type, optimize_centers, ngl)
        self.ac_rcut = ac_rcut_ratio * max(sv.ao_log.sp2rcut)

        lload = (kw['load_from_hdf5'] if 'load_from_hdf5'
                 in kw else False)
        if lload:
            # tests Fortran input
            self.prod_log = prod_log(ao_log=sv.ao_log,
                    sp2charge=sv.sp2charge, tol_loc=tol_loc, load=True)
        else:
            # local basis (for each specie)
    ...
    def chain_data(self):
        """
        This subroutine creates a buffer of information to communicate the
        system variables and the local product vertex to libnao. Later, one
        will be able to generate the bilocal vertex and conversion coefficient
        for a given pair of atom species and their coordinates .
        """
        ...
        (nr, nsp, nmt, nrt) = (aos.nr, aos.nspecies, sum(aos.sp2nmult),
                               aos.nr * sum(aos.sp2nmult))
        (nat, na1, tna, nms) = (sv.natoms, sv.natoms + 1, 3
                                * sv.natoms, sum(aos.sp2nmult)
                                + aos.nspecies)
        (nmtp, nrtp, nmsp) = (sum(pl.sp2nmult), pl.nr
                              * sum(pl.sp2nmult), sum(pl.sp2nmult)
                              + pl.nspecies)
        nvrt = sum(aos.sp2norbs * aos.sp2norbs * pl.sp2norbs)

        ndat = 200 + 2 * nr + 4 * nsp + 2 * nmt + nrt + nms + 3 * 3 \
        ...
        # Simple parameters
        i = 0
        dat[i] = -999.0  # pointer to the empty space in simple parameter
        i += 1
        dat[i] = aos.nspecies
        i += 1
        dat[i] = aos.nr
        ...
        i = 99
        s = 199
        dat[i] = s + 1  # pointer to rr
        i += 1
        f = s + nr
        dat[s:f] = aos.rr
        s = f
        dat[i] = s + 1  # pointer to pp
        i += 1
        f = s + nr
        dat[s:f] = aos.pp
        s = f
        dat[i] = s + 1  # pointer to sp2nmult
        i += 1
        f = s + nsp
        dat[s:f] = aos.sp2nmult
        s = f
        dat[i] = s + 1  # pointer to sp2rcut
        i += 1
        f = s + nsp
        dat[s:f] = aos.sp2rcut
        s = f
        dat[i] = s + 1  # pointer to sp2norbs
        i += 1
        f = s + nsp
        dat[s:f] = aos.sp2norbs
        s = f
        dat[i] = s + 1  # pointer to sp2charge
        i += 1
        f = s + nsp
        dat[s:f] = aos.sp2charge
        s = f
        dat[i] = s + 1  # pointer to sp_mu2j
        i += 1
        f = s + nmt
        dat[s:f] = conc(aos.sp_mu2j)
        s = f
        dat[i] = s + 1  # pointer to sp_mu2rcut
        i += 1
        f = s + nmt
        dat[s:f] = conc(aos.sp_mu2rcut)
        s = f
        dat[i] = s + 1  # pointer to psi_log
        i += 1
        f = s + nrt
        dat[s:f] = conc(aos.psi_log).reshape(nrt)
        s = f
        dat[i] = s + 1  # pointer to sp_mu2s
        i += 1
        f = s + nms
        dat[s:f] = conc(aos.sp_mu2s)
        s = f
        dat[i] = s + 1  # pointer to ucell (123,xyz) ?
        i += 1
        f = s + 3 * 3
        dat[s:f] = conc(sv.ucell)
        s = f
        dat[i] = s + 1  # pointer to atom2sp
        i += 1
        f = s + nat
        dat[s:f] = sv.atom2sp
        s = f
        dat[i] = s + 1  # pointer to atom2s
        i += 1
        f = s + na1
        dat[s:f] = sv.atom2s
        s = f
        dat[i] = s + 1  # pointer to atom2mu_s
        i += 1
        f = s + na1
        dat[s:f] = sv.atom2mu_s
        s = f
        dat[i] = s + 1  # pointer to atom2coord
        i += 1
        f = s + tna
        dat[s:f] = conc(sv.atom2coord)
        s = f
        dat[i] = s + 1  # sp2nmult of product basis
        i += 1
        f = s + nsp
        dat[s:f] = pl.sp2nmult
        s = f
        ...
        dat[s:f] = pl.sp2rcut
        s = f
        dat[i] = s + 1  # sp2norbs of product basis
        i += 1
        f = s + nsp
        dat[s:f] = pl.sp2norbs
        s = f
        ...
        s = f
        dat[i] = s + 1  # pointer to sp2vertex
        i += 1
        f = s + nvrt
    ...
    def comp_apair_pp_libint(self, a1, a2):
        """
        Get's the vertex coefficient and conversion coefficients for a pair
        of atoms given by their atom indices
        """

        from operator import mul
        from pyscf.nao.m_prod_biloc import prod_biloc_c
        
        if not hasattr(self, 'sv_pbloc_data'):
            raise RuntimeError('.sv_pbloc_data is absent')
        assert a1 >= 0
        assert a2 >= 0
        ...
        sv = self.sv
        aos = self.sv.ao_log
        sp12 = np.require(np.array([sv.atom2sp[a] for a in (a1, a2)],
                          dtype=c_int64), requirements='C')
        rc12 = np.require(np.array([sv.atom2coord[a, :] for a in (a1,
                          a2)]), requirements='C')
        icc2a = np.require(np.array(self.ls_contributing(a1, a2),
                           dtype=c_int64), requirements='C')
        npmx = aos.sp2norbs[sv.atom2sp[a1]] \
            * aos.sp2norbs[sv.atom2sp[a2]]
        ...
        libnao.vrtx_cc_apair(
            ...
            rc12.ctypes.data_as(POINTER(c_double)),
            icc2a.ctypes.data_as(POINTER(c_int64)),
            c_int64(len(icc2a)),
            dout.ctypes.data_as(POINTER(c_double)),
            nout,
            )
        ...
        ncc = int(dout[9])
        if ncc != len(icc2a):
            raise RuntimeError('ncc!=len(icc2a)')
        s = 10
        f = s + np.prod(nnn)
        vrtx = dout[s:f].reshape(nnn)
        s = f
    ...
    def get_da2cc_den(self, dtype=np.float64):
        ...
        for (sd, fd, pt, spp) in zip(self.dpc2s, self.dpc2s[1:],
            ...
            for (c, ls, lf) in zip(inf.cc2a, inf.cc2s, inf.cc2s[1:]):
                da2cc[sd:fd, self.c2s[c]:self.c2s[c + 1]] = inf.cc[:,
                        ls:lf]
        return da2cc

    def get_da2cc_nnz(self):
        """
        Computes the number of non-zero matrix elements in the conversion
        matrix ac <=> dp
        """
            ...
            for (c, ls, lf) in zip(inf.cc2a, inf.cc2s, inf.cc2s[1:]):
                for d in range(sd, fd):
                    for a in range(self.c2s[c], self.c2s[c + 1]):
                        (irow[inz], icol[inz], data[inz]) = (d, a,
                                inf.cc[d - sd, a - self.c2s[c] + ls])
                        inz += 1
    ...
    def get_dp_vertex_nnz(self):
        ...
        for (pt, spp) in zip(self.dpc2t, self.dpc2sp):
            if pt == 1:
                nnz += self.prod_log.sp2vertex[spp].size
            elif pt == 2:
                nnz += 2 * self.bp2info[spp].vrtx.size
            else:
    ...
    def get_dp_vertex_doubly_sparse(self, dtype=np.float64, sparseformat=lsofcsr_c,
        ...
        inz = 0
        for (s, f, sd, fd, pt, spp) in zip( atom2s, atom2s[1:], dpc2s,
                                            dpc2s[1:], self.dpc2t, self.dpc2sp):
            ...
            (i1[inz:inz + size], i2[inz:inz + size], i3[inz:inz
             + size], data[inz:inz + size]) = (dd, aa, bb, lv)
            inz += size
    ...
    def get_dp_vertex_doubly_sparse_loops(self, dtype=np.float64,
                                          sparseformat=lsofcsr_c, axis=0,):
        """
        Returns the product vertex coefficients for dominant products as a
        one-dimensional array of sparse matrices, slow version
        """
        ...
        for (atom, [sd, fd, pt, spp]) in enumerate(zip(self.dpc2s,
            ...
            for p in range(sd, fd):
                for a in range(s, f):
                    for b in range(s, f):
                        (i1[inz], i2[inz], i3[inz], data[inz]) = (p, a,
                                b, lv[spp][p - sd, a - s, b - s])
                        inz += 1
            ...
            for p in range(sd, fd):
                for a in range(sa, fa):
                    for b in range(sb, fb):
                        (i1[inz], i2[inz], i3[inz], data[inz]) = (p, a,
                                b, inf.vrtx[p - sd, a - sa, b - sb])
                        inz += 1
                        (i1[inz], i2[inz], i3[inz], data[inz]) = (p, b,
                                a, inf.vrtx[p - sd, a - sa, b - sb])
                        inz += 1
    ...
    def get_dp_vertex_sparse(self, dtype=np.float64,
        ...
        """
        Returns the product vertex coefficients as 3d array for dominant products,
        in a sparse format coo(p,ab) by default
        """
    ...
    def get_dp_vertex_sparse2(self, dtype=np.float64,
        ...
        """
        Returns the product vertex coefficients as 3d array for dominant products,
        in a sparse format coo(pa,b) by default
        """
        ...
        for (sd, fd, pt, spp) in zip(dpc2s, dpc2s[1:], self.dpc2t,
            ...
            (dd, aa, bb) = np.mgrid[sd:fd, sa:fa, sb:fb].reshape((3,
                    size))
            (irow[inz:inz + size], icol[inz:inz + size]) = (dd + aa
                    * nfdp, bb)
            data[inz:inz + size] = inf.vrtx.reshape(size)
            inz += size

            (irow[inz:inz + size], icol[inz:inz + size]) = (dd + bb
                    * nfdp, aa)
            data[inz:inz + size] = inf.vrtx.reshape(size)
            inz += size
        return coo_matrix((data, (irow, icol)), dtype=dtype,
                          shape=(nfdp * n, n))

    def get_dp_vertex_sparse_loops(self, dtype=np.float64,
                                   sparseformat=coo_matrix):
        """
        Returns the product vertex coefficients as 3d array for dominant products,
        in a sparse format coo(p,ab) slow version
        """
        ...
        for (atom, [sd, fd, pt, spp]) in enumerate(zip(self.dpc2s,
            ...
            for p in range(sd, fd):
                for a in range(s, f):
                    for b in range(s, f):
                        (irow[inz], icol[inz], data[inz]) = (p, a + b
                                * n, self.prod_log.sp2vertex[spp][p
                                - sd, a - s, b - s])
                        inz += 1
        ...
        for (atom, [sd, fd, pt, spp]) in enumerate(zip(self.dpc2s,
            ...
            for p in range(sd, fd):
                for a in range(sa, fa):
                    for b in range(sb, fb):
                        (irow[inz], icol[inz], data[inz]) = (p, a + b
                                * n, inf.vrtx[p - sd, a - sa, b - sb])
                        inz += 1
                        (irow[inz], icol[inz], data[inz]) = (p, b + a
                                * n, inf.vrtx[p - sd, a - sa, b - sb])
                        inz += 1
        return sparseformat((data, (irow, icol)), dtype=dtype,
                            shape=(nfdp, n * n))
    ...
    def init_c2s_domiprod(self):
        ...
        #  product Center -> list of the size of the basis set in this center,
        # of center's types,of product species
        (c2n, c2t, c2sp) = ([], [], [])
        for (atom, sp) in enumerate(self.sv.atom2sp):
            c2n.append(self.prod_log.sp2vertex[sp].shape[0])
            c2t.append(1)
            c2sp.append(sp)
        for (ibp, inf) in enumerate(self.bp2info):
            c2n.append(inf.vrtx.shape[0])
            c2t.append(2)
            c2sp.append(ibp)

        # number of product centers in this vertex
        ndpc = len(c2n)
    ...
    def comp_moments(self, dtype=np.float64):
        """
        Computes the scalar and dipole moments for the all functions in the
        product basis
        """
    ...
    def overlap_check(self, **kw):
        ...
        return [[abs(sref - sprd).sum() / sref.size, np.amax(abs(sref
                - sprd))]]
    ...
    def dipole_check(self, **kw):
        ...
        for (i, dref) in enumerate(dipcoo):
            ...
            xyz2err.append([abs(dprd - dref).sum() / dref.size,
    ...
    def generate_png_spy_dp_vertex(self):
        """
        Produces pictures of the dominant product vertex in a common 
        black-and-white way
        """
    ...
    def generate_png_chess_dp_vertex(self):
        ...
        for (i, ab) in enumerate(dab2v):
            ...
            print('Matrix No.#{}, Size: {}, Type: {}'.format(i + 1,
                  ab.shape, type(ab)), fname)
            if type(ab) != 'numpy.ndarray':
                ab = ab.toarray()
            fig = plt.figure()
    ...
    def reshape_COO(a, shape):
        """
        Reshape the sparse matrix (a) and returns a coo_matrix with
        favourable shape.
        """

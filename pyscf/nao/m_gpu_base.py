...
class gpu_initialize():

    def __init__(self, GPU, norbs, nfermi, nprod, vstart):
        """
          Input Parameters:
          -----------------
        ...
        if (isinstance(GPU, dict) or GPU == True) and GPU_import:
            ...
            if not self.GPU["use"]:
              ...
              # threads by block
              self.block_size = np.array([32, 32], dtype=np.int32)
              
              # number of blocks
              self.grid_size = np.array([0, 0], dtype=np.int32)
